	...
	public boolean onCommand(CommandSender sender, Command arg1, String arg2, String[] arg3) {
		...
		if(sender instanceof Player && arg3.length == 1) {
			...
			if(inhand != null && arg3.length == 1 && receiver != null) {
				player.sendMessage("¡ìbGift Successfully Sent!");
				player.getInventory().setItemInMainHand(null);
				newmeta.setLore(newlore);
				inhand.setItemMeta(newmeta);
				receiver.getInventory().addItem(inhand);
				receiver.sendMessage("¡ìbYou received a gift from " + ChatColor.AQUA + player.getName() + ChatColor.AQUA + "!");
			}
			else {
				player.sendMessage("¡ìcFailed to send gift. Check parameters.");
				return false;
			}
		}
		return true;
